...
<head>
    <title>KMS {{ $jenisKelamin }} - {{ $anak->nama_lengkap }}</title>
    <style>
        @page {
            size: A3 landscape;
            margin: 1cm;
...
<body>
    <div class="header">
        <div class="header-title">KMS</div>
        <div class="header-subtitle">KARTU MENUJU SEHAT - Untuk {{ $jenisKelamin }}</div>
    </div>
    ...
    <div style="text-align: center; font-style: italic; margin-bottom: 5px;">
        Timbanglah Anak Anda Setiap Bulan<br>
    ...
    <!-- Tabel Data Horizontal dengan Merge Cell untuk KBM -->
    ...
    <table class="kms-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                @for ($i = 0; $i <= 24; $i++)
                    @php
                        $currentKbm = $i > 0 ? app('App\Models\StandarKms')::getKbm($anak->jenis_kelamin, $i) : null;

                        if ($i > 0 && $currentKbm == $prevKbm) {
                            $mergeCount++;
                        } else {
                            if ($mergeCount > 1) {
            ...
            <!-- Baris ASI Eksklusif -->
            <tr>
                <td class="row-header">ASI Eksklusif</td>
                @for ($i = 0; $i <= 24; $i++)
                    <td>
                        @php
                            $timbangan = $anak->timbangan->firstWhere('umur_bulan', $i);
                            echo $timbangan && $timbangan->asi_eksklusif ? '✓' : '';
                        @endphp
                    </td>
                @endfor
            </tr>
    ...
    {{-- <div class="horizontal-table">
        <!-- Header Bulan -->
        <div class="month-column month-header">Bulan</div>
        @for ($i = 0; $i <= 12; $i++)
            <div class="month-column month-header">{{ $i }}</div>
        @endfor
    </div>

    <div class="horizontal-table">
        <div class="month-column">BB (kg)</div>
        @for ($i = 0; $i <= 12; $i++)
            @php
                $timbangan = $anak->timbangan->firstWhere('umur_bulan', $i);
            @endphp
            <div class="month-column">{{ $timbangan ? number_format($timbangan->berat, 1) : '-' }}</div>
        @endfor
    </div>

    <div class="horizontal-table">
        <div class="month-column">KBM (gr)</div>
        @for ($i = 0; $i <= 12; $i++)
            <div class="month-column">
                @if ($i > 0)
                    @php
                        $kbm = app('App\Models\StandarKms')::getKbm($anak->jenis_kelamin, $i);
                        echo $kbm ?? '-';
                    @endphp
                @else
                    -
    ...
    <div class="horizontal-table">
        <div class="month-column">N/T</div>
        @for ($i = 0; $i <= 12; $i++)
            @php
                $timbangan = $anak->timbangan->firstWhere('umur_bulan', $i);
            @endphp
            <div class="month-column">{{ $timbangan ? $timbangan->kategori_berat : '-' }}</div>
        @endfor
    </div> --}}
    ...
    <div class="interpretasi">
        ...
        <div style="font-style: italic;">KBM digunakan bila ada keraguan menginterpretasikan arah kurva pertumbuhan
        </div>
    </div>
    ...
    <div class="interpretasi" style="border-top: none; padding-top: 0;">
        ...
        <div>- Di Bawah Garis Merah</div>
        <div>- Di Atas Garis Oranye</div>
    </div>
